function road(){
			...
			*/
			...
			this.speedIntervals = 10; //ivana, to vary states faster
			...
			this.agentCar ={
				speed: 20,
				oldspeed: 0, //ivana
				prevAction: 0, //ivana, so can store and revert to previous speed after randomly testing out action "stop"
				overtaking : false,
				overtakingSteps:0,
				collisionTime: 0,
			...
			this.exploration = 0.05;// ivana: pretty small value for exploration? can we start big, and reduce to small after x steps?
		...
		road.prototype.applyAction = function(action){
			if(action == 1)
			{
				...
				this.agentCar.prevAction=1; //ivana
			}
			else if(action == 2) {
				//stop 
				this.agentCar.oldspeed =this.agentCar.speed;
				this.agentCar.speed = 0;
				this.agentCar.prevAction=2;//ivana
				...
				this.setPosition(2);
				this.agentCar.prevAction=3;//ivana
			}
			else if(action == 4){
				//slow down
				...
				this.agentCar.prevAction=4;//ivana
			}
			else if(action == 5){
				//speed up
				this.agentCar.speed += this.speedIntervals;
				this.agentCar.prevAction=5;//ivana
			}
			...
			//ivana : got rid of staying at 0 action as it never gets back to normal range in 5km intervals
		/*	if(action != 2){
				//if set to 60 every time then doesnt explore other ranges?
				//this.agentCar.speed =60;
				//if do this incoming cars change speed too? go backwards??
				//this.agentCar.speed = ~~(Math.random()*60);
				this.agentCar.speed =this.agentCar.oldspeed;
			}
			*/
		}
		...
		function step(){
				...
				//ivana: to control exploration vs exploitation
			exploring = true;
			steps++;
			//ivana
			...
			//ivana: what should do here is take random actions for the first half, and then egreedy (ie best always) otherwise
			...
			//ivana: if doesnt know best action, then explores? what does it mean to "know"? does it ever not know the action? if always knows, can leave
			...
			//ivana = random action causes a bug with no new cars generated?
			if (exploring)
			{		
				action=randomAction;
			}
			...
			*/
			...
			//ivana
			summary += "<br /> Exploring: " + exploring;
