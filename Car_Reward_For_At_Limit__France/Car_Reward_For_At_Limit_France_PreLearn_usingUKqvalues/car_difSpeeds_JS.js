function road(){
			...
			*/
			...
			this.agentCar ={
				...
				position: {
					line: 1, //for france. put back to 1 for uk, so thats it starts wrong!
		        	column: 3
				}
			};
		...
		road.prototype.applyAction = function(action){
			if(action == 1)
			{
				...
				this.setPosition(2);//changed to 1, left lane. move back to 2, now for ukl
			}
			else if(action == 4){
				//slow down
			...
			if(action != 3){
				this.setPosition(1); //if it's not overtaking move it into the left lane/changed to right. move back to 1, for france
		...
		road.prototype.checkIfCrashed = function(){
			if (this.agentCar.position.line == 2){//changed to right lane for france
		...
		//check collision time and lane BEFORE applying action!
			...
			if ((game.agentCar.collisionTime== 0)&&(game.agentCar.lane="right"))
				wrongOvertake=true;
